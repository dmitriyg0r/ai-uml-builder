...
import { AlignLeft, Check, Play, AlertTriangle, Wand2, Loader } from 'lucide-react';
...
// Icons
const FormatIcon = () => <AlignLeft className="w-3.5 h-3.5" />;
const CheckIcon = () => <Check className="w-3 h-3" />;
const PlayIcon = () => <Play className="w-4 h-4" />;
const AlertIcon = () => <AlertTriangle className="w-3 h-3" />;
const MagicWandIcon = () => <Wand2 className="w-3.5 h-3.5" />;
const SpinnerIcon = () => <Loader className="w-3.5 h-3.5 animate-spin" />;
...
export const CodeEditor: React.FC<CodeEditorProps> = React.memo(({ value, onChange, onRun, onFixCode }) => {
  ...
  ), [validationStatus, lineCount, hasChanges, t, localValue, handleFormat, handleFixCode, isFixing, handleRun, onFixCode]);
